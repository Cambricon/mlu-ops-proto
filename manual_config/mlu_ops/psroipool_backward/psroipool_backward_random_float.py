...
def genSingleCase(dtype='float32', params_list=[1,1,2,2,1]):
    ...
    bottom_h = np.random.randint(10, 5*h)
    bottom_w = np.random.randint(10, 5*w)
    ...
    inputs = '      {"inputs":['
    input1 = '{' + dShape(top_grad_shape) + ',' + dType(dtype) + ',' + dRandomDistribution(-100,100) + "," + dlayout("NHWC") + '}'
...
if __name__ == "__main__":
    ...
    res += genCase()
    file = open("./psroipool_backward_manual_random_float32.json",'w')
    file.write(res)
    file.close()
