...
def genSingleCase(dtype='float32', params_list=[1,1,2,2,1]):
    ...
    output_shape = [rois_num*n, pooled_height, pooled_width, ci]
    inputs = '      {"inputs":['
    input1 = '{' + dShape(input_shape) + ',' + dType(dtype) + ',' + dRandomDistribution(-10,10) + ","+'"contain_nan":true,"contain_inf":true'+ ','+ dlayout("NHWC") + '}'
    input2 = '{' + dShape(grid_shape) + ',' + dType(dtype) + ',' + dRandomDistribution(-1,1) + ',' + dlayout("ARRAY") + '}'
    outputs = '      "outputs":['
    output1 = '{' + dShape(output_shape) + ',' + dType(dtype) + ',' + dRandomDistribution(-10,10) + ',' + dlayout("NHWC") + '}'
...
if __name__ == "__main__":
    ...
    res += genCase()
    file = open("./roi_crop_forward_manual_random_float32_nan_and_inf.json",'w')
    file.write(res)
    file.close()
