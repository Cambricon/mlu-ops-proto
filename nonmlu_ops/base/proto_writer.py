...
def writeData2File(tensor_data, tensor_data_real, tensor_data_imag, dtype_str,
                   file_name):
    ...
    if dtype_str in ['int8', 'uint8', 'bool']:
        ...
        concat_data = np.stack((tensor_data_real, tensor_data_imag),
                               -1).flatten()
...
class ProtoWriter:
    ...
    def saveNode2File(self):
        """Change params to pb params.

        If kernel own params for themselves, please rewrite this function.
        """
        # save pb file
    ...
    def dumpTensor2Node(self):
        ...
        '''
        for id, output_tensor in enumerate(
                self.tensor_list_.getOutputTensors()):
    ...
    def dumpPublicParam2Node(self):
        ...
        if not self.op_params_.get('if_dynamic_threshold', False):
            ...
            if evaluation_criterion_num == evaluation_threshold_num:
                for threshold in self.proto_params_.get(
    ...
    def writeData2Node(self, tensor_node, tensor_data, tensor_data_real,
                       tensor_data_imag, dtype_str):
        # store float data as int to speed up gtest parser pb/prototxt file and
        # save store space
    ...
    def writeTensor(self, tensor_node, tensor_name, tensor):
        ...
        if tensor.require_value_ and data_exist:
            ...
            if self.proto_params_.get('large_tensor', False):
                ...
                writeData2File(tensor_data, tensor_value_real,
                               tensor_value_imag, data_type_str,
                               self.folder_name_ + '/' + data_file_name)
            else:
                self.writeData2Node(tensor_node, tensor_data,
                                    tensor_value_real, tensor_value_imag,
                                    data_type_str)
