...
class EvaluatorImpl():
    ...
    def computeDiff2(self, base_data, compute_data):
        ...
        self.checkNan(base_data, compute_data)
        self.diff2_ = np.sqrt(
            np.sum(np.square(base_data - compute_data)) /
...
class Evaluator():
    def __init__(self,
                 base_datanode,
                 compute_datanode,
                 half_min_threshold=0,
                 float_min_threshold=0,
                 check_rate=True):
        ...
        self.eva_impl_ = EvaluatorImpl(dtype_ulp=self.dtype_ulp_,
                                       is_float_diff=self.is_float_diff_)
    ...
    def computeDynamicDiff(self, diff, rate):
        """diff should be within [min_threshold, max_threshold] here, static
        threshold is minimum_threshold if rate is over default rate,
        minimum_threshold should be extended correspondingly."""
    ...
    def computeDiffBase(self, diff_func, post_process_func, rate):
        ...
        if self.is_complex_:
            ...
            if 0 in [
                    self.base_data_.size, self.base_imag_.size,
                    self.compute_real_.size, self.compute_imag_.size
            ]:
                ...
                self.base_real_ = base_real.astype(compute_node_dtype).astype(
                    np.float64)
                self.base_imag_ = base_imag.astype(compute_node_dtype).astype(
                    np.float64)
            ...
            if self.base_data_.size == 0 or self.compute_data_.size == 0:
                ...
                self.compute_data_ = self.compute_datanode_.getData().astype(
                    np.float64)
    ...
    def computeDiff1(self, rate=10.0):
        ...
        diff1 = self.computeDiffBase(self.eva_impl_.computeDiff1,
                                     self.computeDynamicDiff, rate)
        if self.check_rate_:
            assert diff1 <= self.diff_check_, '[Evaluator]: unexcepted dynamic diff1 threshold {}, which is too large.\
                Please check whether base_data and compute data are correct'.format(
                diff1)
        return diff1
    ...
    def computeDiff2(self, rate=10.0):
        ...
        diff2 = self.computeDiffBase(self.eva_impl_.computeDiff2,
                                     self.computeDynamicDiff, rate)
        if self.check_rate_:
    ...
    def computeDiff3(self, rate=10.0):
        ...
        diff3 = self.computeDiffBase(self.eva_impl_.computeDiff3,
                                     computeDynamicDiff3, rate)
        return diff3
    ...
    def computeDiff3_2(self, rate=10.0):
        ...
        diff3_2 = self.computeDiffBase(self.eva_impl_.computeDiff3,
                                       computeDynamicDiff3_2, rate)
        return diff3_2
