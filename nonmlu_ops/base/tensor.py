...
class DataType(Enum):
    ...
    def isComplex(self):
        complex_list = [DataType.COMPLEX_HALF, DataType.COMPLEX_FLOAT]
...
class DataNode:
    ...
    def __repr__(self):
        ...
        return (f'{DataNode.__module__}.{DataNode.__qualname__} '
                f'dtype: {self.dtype_}, position: {self.position_}' +
                shape_str)
...
class DiffInfo():
    ...
    def __str__(self):
        if self.imag_diff_ == -1:
            return f'DiffInfo: {self.real_diff_}'
        else:
            return f'DiffInfo: ({self.real_diff_},{self.imag_diff_})'
        ...
        if dtype == DataType.BOOL:
            if distribution == 'uniform':
                if [start, end] not in [[0, 1], [0, 2], [1, 2]]:
                    start, end = 0, 2
                np_value = np.array(np.random.randint(start, end, size=shape),
                                    dtype='bool')
            elif distribution == 'binomial':
                np_value = np.random.binomial(start, end,
                                              size=shape).astype(dtype='bool')
        else:
            if distribution == 'uniform':
                if dtype.isComplex():
                ...
                if dtype.isComplex():
                    ...
                    np_data = np.random.normal(
                        start, end / sqrt(2),
                        size=complex_shape).astype(random_dtype)
                    real_data, imag_data = np_data[..., 0], np_data[..., 1]
                    np_complex_value_real, np_complex_value_imag = np_data[
                        ..., 0], np_data[..., 1]
                else:
                ...
                ), 'complex type not support sample random'
                np_value = np.array(random.sample(range(start, end), end))

            elif distribution == 'binomial':
                assert not dtype.isComplex(
                ), 'complex type not support binomial random'
                np_value = np.random.binomial(start, end,
                                              size=shape).astype(random_dtype)

        if dtype.isFloatPoint() and self.tensor_.hasNanInf():
            np_value = quantize_utils.fillNanInf(np_value, dtype,
                                                 self.tensor_.contain_nan_,
                                                 self.tensor_.contain_inf_)

        return np_value, np_complex_value_real, np_complex_value_imag, position


class TensorList:
    """TensorList is used for generating data for input tensors and saving
    them.
    ...
    def __len__(self):
        """return nums of tensors in tensorlist, contain input and output
        tensors."""
    ...
    def run(self):
        """Main process of TensorList Normally do not need rewrite this
        function."""
        self.preProcess()
        self.generateData()
        self.castDataNode()
    ...
    def preProcess(self):
        """for adjusting param inherite this class and rewrite function
        preProcess."""
    ...
    def generateData(self):
        ...
        If tensor has attribute filename, data will be loaded from file, else
    ...
    def castDataNode(self):
        """Cast input data to onchip data by input_dtype and
        input_onchip_dtype."""
        for input_tensor in self.input_tensors_:
            quantize_utils.caseDataNode(input_tensor.onchip_datanode_,
                                        input_tensor.getDataNode())
