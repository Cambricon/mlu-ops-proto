...
class InputFileParser():
    """Class for parser input file to tensor params, op params and proto
    params.
...
class InputFileParser_json(InputFileParser):
    ...
    def parser_json(self):
        with open(self.file_name_) as file:
    ...
    def parseGlobalParam(self):
        ...
        global_tensor_param_key = [
            'data_type', 'random_distribution', 'require_value', 'if_scale',
            'if_offset', 'contain_nan', 'contain_inf'
        ]
        global_op_param_key = [
            'op_name', 'if_dynamic_threshold', 'supported_mlu_platform'
        ]
        global_proto_param_key = [
            'device', 'evaluation_criterion', 'evaluation_threshold',
            'supported_mlu_platform', 'large_tensor'
        ]
    ...
    def genCase(self):
        ...
        If call this function repeatedly, it will return all cases which can
        parser from the input manual json. Then it will return an empty dict.
        """
        ...
        for i in range(len(inputs)):
            ...
            input_param.setdefault(
                'dtype',
                data_type.get('input_dtype', input_default_dtype)[i])
            input_param.setdefault(
                'onchip_dtype',
                data_type.get('input_onchip_dtype', input_default_dtype)[i])
        ...
        for i in range(len(outputs)):
            ...
            output_param.setdefault(
                'dtype',
                data_type.get('output_dtype', output_default_dtype)[i])
            output_param.setdefault(
                'onchip_dtype',
                data_type.get('output_onchip_dtype', output_default_dtype)[i])
        ...
        case_op_param.update(self.manual_params_[self.case_count_].get(
            'op_params', {}))
        ...
        case_proto_param.update(self.manual_params_[self.case_count_].get(
            'proto_params', {}))
...
class InputFileParser_prototxt(InputFileParser):
    ...
    def parser_prototxt(self):
        with open(self.file_name_) as openfile:
    ...
    def parsePrototxtParam(self):
        ...
        for index, val in enumerate(inputs):
            ...
            random_distribution_type = enum2str(
                mlu_op_test_pb2.RandomDistribution.Name(
                    val.random_data.distribution))
        ...
        for index, val_out in enumerate(outputs):
            ...
            random_distribution_type = enum2str(
                mlu_op_test_pb2.RandomDistribution.Name(
                    val_out.random_data.distribution))
            random_distribution_low_up = [
                val_out.random_data.lower_bound,
                val_out.random_data.upper_bound
            ]
            random_distribution.update(
                {random_distribution_type: random_distribution_low_up})
        ...
        case_proto_param = {}
        case_proto_param['device'] = enum2str(
            mlu_op_test_pb2.Device.Name(
        ...
        case_proto_param[
            'evaluation_threshold'] = self.get_parse_infor.test_param.error_threshold
        case_proto_param['write_data'] = True
        case = ProtoReadFactory().factory(case_op_param['op_name'])(
            self.get_parse_infor, case_tensor_param, case_op_param,
            case_proto_param, case).run()
        return case
