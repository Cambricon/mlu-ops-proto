...
class MluOpBuilder(Builder):
    ...
    def registerOp2Factory(self):
        ...
        for register_op_name in self.args_.op_name:
            register_op_dir = Path(cambricon_dir + '/nonmlu_ops/' +
                                   register_op_name)
    ...
    def run(self):
        ...
        if not os.environ.get(
                'CUDA_VISIBLE_DEVICES') and utils.available_nvidia_smi():
    ...
    def processJsonFile(self):
        ...
        for file, op_name_register in self.file_dict_.items():
            input_parser = InputFileParser_json(file, self.args_.file_type,
                                                self.arg_params_)
    ...
    def processPrototxtFile(self):
        ...
        for file, op_name_register in self.file_dict_.items():
            input_parser = InputFileParser_prototxt(file, self.args_.file_type,
                                                    self.args_params_)
...
class MluOpGenerator:
    ...
    def run(self):
        ...
        op_test.run()
        proto_writer = ProtoWriterFactory().factory(op_name)(tensor_list,
                                                             self.op_param_,
                                                             self.proto_param_)
